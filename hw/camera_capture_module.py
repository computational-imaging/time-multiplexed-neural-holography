...
class CameraCapture:
    def __init__(self, params):
        ...
        self.demosaick_rule = cv2.COLOR_BAYER_BG2RGB # RGGB to RGB, Grasshopper3, U3, projector
        self.params = params
    ...
    def set_shutter_speed(self, val):
        self.camera_device.setProperty(type = PyCapture2.PROPERTY_TYPE.SHUTTER, autoManualMode = False, absValue = val)
        shutter_speed = self.camera_device.getProperty(PyCapture2.PROPERTY_TYPE.SHUTTER ).absValue
        print(f"Shutter speed set to {shutter_speed}ms")
    
    def set_gain(self, val):
        self.camera_device.setProperty(type = PyCapture2.PROPERTY_TYPE.GAIN, autoManualMode = False, absValue = val)
        gain = self.camera_device.getProperty(PyCapture2.PROPERTY_TYPE.GAIN).absValue
