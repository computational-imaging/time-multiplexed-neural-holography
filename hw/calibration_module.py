...
def circle_detect(captured_img, num_circles, spacing, pad_pixels=(0., 0.), show_preview=True, quadratic=False):
    ...
    img = (np.clip(captured_img.copy(), 0, 1) * 255).astype(np.uint8)
    print(img[...,0].mean())
    print(img[...,1].mean())
    print(img[...,2].mean())
    if len(img.shape) > 2:
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        cv2.imwrite("temp/img_gray.png", img)
    print(img[...,0].mean())
    print(img[...,1].mean())
    print(img[...,2].mean())


    img = cv2.medianBlur(img, 5)  # Red 71
    # cv2.imwrite("temp/img_blur.png", img)
    img_gray = img.copy()

    img = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 201, 0)
    cv2.imwrite("temp/img_adapt_thres.png", img)

    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    img = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel)
    cv2.imwrite("temp/img_open.png", img)
    img = 255 - img
    ...
    params.minCircularity = 0.5 # change here, easier to detect blob
    ...
    if found_dots:
        ...
        homography_cache = {'H':H, 'centers':centers}
        savemat(f'./cache_h.mat', homography_cache)
    else:
        print("No dots")
        homography_cache = loadmat(f'./cache_h.mat')
        H = homography_cache['H']
        centers = homography_cache['centers']
    ...
    if show_preview:
        ...
        ax = fig.add_subplot(223) # grayscale 
        ax.imshow(img_gray, cmap='gray')

        ax2 = fig.add_subplot(221) # binarized image
        ax2.imshow(img, cmap='gray')

        ax3 = fig.add_subplot(222) # captured image
