...
from props.prop_submodules import Field2Input, Output2Field, create_gaussian, \
    ContentDependentField, LatentCodedMLP, SourceAmplitude, Conv2dField
...
def model(opt, dev=torch.device('cuda'), preload_H=False):
    ...
    if opt.prop_model.lower() == 'asm':
        ...
        sim_prop = CNNpropCNN(opt.prop_dist, opt.wavelength, opt.feature_size,
                              ...
                              lut0=opt.lut0,
                              param_lut=opt.param_lut,
                              slm_latent_amp=opt.slm_latent_amp,
                              slm_latent_phase=opt.slm_latent_phase,
                              fourier_latent_amp=opt.f_latent_amp,
                              fourier_latent_phase=opt.f_latent_phase,
                              share_F_amp=opt.share_f_amp,
                              share_F_phase=opt.share_f_phase,
                              flipud_slm=opt.flipud,
                              loss_func=opt.loss_train,
                              lr=opt.lr,
                              plane_idxs=opt.plane_idxs,
                              learn_f_amp_wrp=opt.learn_f_amp_wrp,
                              learn_f_phase_wrp=opt.learn_f_phase_wrp,
                              slm_cnn_residual=opt.slm_cnn_residual,
                              target_cnn_residual=opt.target_cnn_residual,
                              min_mse_scaling=opt.min_mse_scaling
                              ).to(dev)

    if opt.prop_model_path is not None:
        checkpoint = torch.load(opt.prop_model_path)
        sim_prop.load_state_dict(checkpoint["state_dict"], strict=False)
        sim_prop.eval()
...
class PropModel(pl.LightningModule):
    ...
    :param lr:
    """
    def __init__(self, roi_res=(800, 1280), flipud_slm=False, plane_idxs=None, 
                loss_func=F.l1_loss, lr=4e-4, min_mse_scaling=False):
        super(PropModel, self).__init__()
        self.roi_res = roi_res
        self.flipud_slm = flipud_slm
        self.plane_idxs = plane_idxs
        self.loss_func = loss_func # reconstruction loss function
        self.lr = lr
        self.recon_amp = {}
        self.target_amp = {}
        
        self.min_mse_scaling = min_mse_scaling

    def perform_step(self, batch, batch_idx, prefix):
        one_hot, target_amps = batch
        if one_hot.shape[1] > 1 and self.lut is not None:
            # one-hot encoding that allows to parameterize LUT.
            phase = (one_hot * self.lut).sum(1, keepdims=True)
        else:
            phase = one_hot

        if self.flipud_slm:
            # TI SLM flips its pattern vertically before displaying;
            slm_phase = phase.flip(dims=[2])
        else:
            slm_phase = phase
        ...
        # calculate losses
        # pixel reconstruction mse loss
        loss, loss_mse, loss_mse_all = self.mp_loss(recon_fields, target_amps,
                                                    self.plane_idxs, self.loss_func, prefix, self.min_mse_scaling)
        ...
        with torch.no_grad():
            ...
            # total loss
            self.log(f'loss_total_{prefix}', loss_total, on_step=True, on_epoch=True)

        return loss_total

    def mp_loss(self, recon_fields, target_amps, plane_idxs, loss_func, prefix, min_mse_scaling):
        ...
        if min_mse_scaling:
            with torch.no_grad():
                s = (recon_amps * target_amps).mean() / (recon_amps**2).mean()
            recon_amps = s * recon_amps
...
class CNNpropCNN(PropModel):
    ...
    :param lut0: look up table.
    :param param_lut: If True, parameterize a look up table.
    :param slm_latent_amp: If True, parameterize the amplitude at SLM plane.
    :param slm_latent_phase: If True, parameterize the phase at SLM plane.
    :param fourier_latent_amp: If True, parameterize the amplitude at Target plane.
    :param fourier_latent_phase: If True, parameterize the phase at Target plane.
    :param f_amp_init_th: 0.36
    :param share_F_amp: If True, share the amplitude parameters at Fourier plane.
    :param share_F_phase: If True, share the phase parameters at Fourier plane.
    :param flipud_slm: If True, flip the slm phase before propagation (which TI SLM does).
    ...
    def __init__(self, prop_dist, wavelength, feature_size, prop_type='ASM', F_aperture=1.0,
                 prop_dists_from_wrp=None, linear_conv=True, slm_res=(800, 1280), roi_res=(700, 1190),
                 num_downs_slm=0, num_feats_slm_min=0, num_feats_slm_max=0,
                 num_downs_target=0, num_feats_target_min=0, num_feats_target_max=0,
                 norm=nn.InstanceNorm2d, slm_coord='both', target_coord='both',
                 lut0=None, param_lut=False, slm_latent_amp=False, slm_latent_phase=False,
                 fourier_latent_amp=False, fourier_latent_phase=False, f_amp_init_th=0.36,
                 share_F_amp=False, share_F_phase=False, flipud_slm=False, loss_func=F.l1_loss, lr=4e-4,
                 plane_idxs=None, learn_f_amp_wrp=False, learn_f_phase_wrp=False, 
                 slm_cnn_residual=False, target_cnn_residual=False, min_mse_scaling=False
                ):
        super(CNNpropCNN, self).__init__(roi_res=roi_res, flipud_slm=flipud_slm,
        ...
        self.f_latent_amp = None
        self.f_latent_phase = None

        # LUT
        if lut0 is not None:
            lut = lut0.requires_grad_(param_lut)
            self.lut = nn.Parameter(lut.reshape(1, len(lut), 1, 1), requires_grad=param_lut)
        ...
        # Learned Fourier amp/phase
        if fourier_latent_amp:
        ...
        self.slm_cnn_residual = slm_cnn_residual
        ...
        if prop_dists_from_wrp is not None:
            self.f_latent_amp_wrp = self.f_latent_amp if share_F_amp else None
            self.f_latent_phase_wrp = self.f_latent_phase if share_F_phase else None
            if learn_f_amp_wrp:
                self.f_latent_amp_wrp = nn.Parameter(torch.ones(1, len(prop_dists_from_wrp),
                                                                *fourier_res, requires_grad=True))
            if learn_f_phase_wrp:
        ...
        self.target_cnn_residual = target_cnn_residual
        
    def forward(self, field):
        #print("field", field.shape)
        if self.slm_cnn is not None:
            if self.slm_cnn_residual:
                #print("slm residual")
                slm_field = self.slm_cnn(field) + field
            else:
        ...
        if self.target_cnn is not None:
            ...
            if self.target_cnn_residual:
                output_field = target_field + output_field
        else:
    ...
    def perform_step(self, batch, batch_idx, prefix):
        loss = super(CNNpropCNN, self).perform_step(batch, batch_idx, prefix)

        # Model visualization convergence
        #data_dict = {}
        #if self.global_step % 400 == 0:
        #    for name, param in self.named_parameters():
        #        data_dict[name] = param.squeeze().cpu().detach().numpy()
        #    sio.savemat(os.path.join(f'./temp/{self.global_step}.mat'), data_dict)
    ...
    def plot_lut(self, prefix):
        """
        plot a bar chart with parameterized LUT
        """
        fig = plt.figure()
        plt.bar(range(16), (self.lut - self.lut.min()).squeeze().cpu().detach().numpy(), 0.6)
        self.logger.experiment.add_figure(f'Parameterized_lut/{prefix}', fig, self.global_step)
...
class PropNH(PropModel):
    ...
    """
    def __init__(self, prop_dist, wavelength, feature_size, prop_type='ASM', F_aperture=1.0,
                 prop_dists_from_wrp=None, linear_conv=True, slm_res=(800, 1280),
                 roi_res=(800, 1280), plane_idxs=None, loss_func=F.l1_loss, lr=4e-4,
                 num_gaussians=3, init_sigma=(500.0, 700.0, 900.0), init_amp=0.9,
        ...
        if lut0 is not None:
            self.lut = lut0.reshape(1, len(lut0), 1, 1)
    ...
    def perform_step(self, batch, batch_idx, prefix):
        loss = super(PropNH, self).perform_step(batch, batch_idx, prefix)
